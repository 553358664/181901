  ...
  <div class="exit">
    <ul class="exitcontent">
      <li v-for="(item,index) in attention" :key="index">
        <User-com>
          <img class="portrait" slot="userImg" :src="item.userPhoto">
        ...
        <Article-com>
          ...
          <!-- <span class="commenter" slot="commenter2">{{item.commentList[1].userName}}:</span>
          <span class="content" slot="comment2">{{item.commentList[1].articleContent}}</span> -->
...
export default {
  computed: {
    ...Vuex.mapState({
        ...
        commentList:state=>state.community.commentList
    })
  },
  components: {
    "User-com": User,
